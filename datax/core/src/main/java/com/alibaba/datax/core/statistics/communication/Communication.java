...
public class Communication extends BaseObject implements Cloneable {
    ...
    public synchronized Communication mergeFrom(final Communication otherComm) {
        ...
        if (this.startTimestamp > otherComm.getStartTimestamp()) {
			this.startTimestamp = otherComm.getStartTimestamp();
		}
        
        if (otherComm.getEndTimestamp() > this.endTimestamp) {
			this.endTimestamp = otherComm.getEndTimestamp();
		}
