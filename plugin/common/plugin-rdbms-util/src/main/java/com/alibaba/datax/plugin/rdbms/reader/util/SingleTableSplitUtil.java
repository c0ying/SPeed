...
import com.alibaba.datax.plugin.rdbms.util.RdbmsRangeSplitWrap.SplitInfoWrap;
...
//
//    public SingleTableSplitUtil(DataBaseType dataBaseType) {
//    	this.dataBaseType = dataBaseType;
//    }

    public static List<Configuration> splitRangeTable(
            Configuration configuration, SplitInfoWrap splitInfoWrap, int adviceNum, DataBaseType dataBaseType) {
    	List<Configuration> pluginParams = new ArrayList<Configuration>();
    	List<SplitInfoWrap> rangeList;
        ...
        rangeList = RdbmsRangeSplitWrap.splitAndWrapInfo(splitInfoWrap, adviceNum, splitPkName, "", dataBaseType);
        
        String tempQuerySql;
        List<String> allQuerySql = new ArrayList<String>();

        if (null != rangeList && !rangeList.isEmpty()) {
            for (SplitInfoWrap range : rangeList) {
                Configuration tempConfig = configuration.clone();

                tempQuerySql = buildQuerySql(column, table, where)
                        + (hasWhere ? " and " : " where ") + range.getRangeSQL();

                allQuerySql.add(tempQuerySql);
                tempConfig.set(Key.QUERY_SQL, tempQuerySql);
                tempConfig.set(Key.SPLIT_LOWER, range.getRangePair().getLeft());
                tempConfig.set(Key.SPLIT_UPPER, range.getRangePair().getRight());
                tempConfig.set(Key.SPLIT_LOWER_EQ, range.getEqualPair().getLeft());
                tempConfig.set(Key.SPLIT_UPPER_EQ, range.getEqualPair().getRight());
    ...
    public static List<Configuration> splitSingleTable(
            Configuration configuration, int adviceNum, DataBaseType dataBaseType) {
        List<Configuration> pluginParams = new ArrayList<Configuration>();
...
//                    configuration, adviceNum,dataBaseType);
            // warn: mysql etc to be added...
//        } else {
            Pair<Object, Object> minMaxPK = getPkRange(configuration,dataBaseType);
            if (null == minMaxPK) {
            ...
            if (isStringType) {
                rangeList = RdbmsRangeSplitWrap.splitAndWrapInfo(
...
//        }
    ...
    private static Pair<Object, Object> getPkRange(Configuration configuration,DataBaseType dataBaseType) {
        ...
        Pair<Object, Object> minMaxPK = checkSplitPk(conn, pkRangeSQL, fetchSize, table, username, configuration,dataBaseType);
        DBUtil.closeDBResources(null, null, conn);
        return minMaxPK;
    }

    public static void precheckSplitPk(Connection conn, String pkRangeSQL, int fetchSize,
                                                       String table, String username, DataBaseType dataBaseType) {
        Pair<Object, Object> minMaxPK = checkSplitPk(conn, pkRangeSQL, fetchSize, table, username, null,dataBaseType);
        if (null == minMaxPK) {
            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                    "根据切分主键切分表失败. DataX 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
        }
    ...
    private static Pair<Object, Object> checkSplitPk(Connection conn, String pkRangeSQL, int fetchSize,  String table,
                                                     String username, Configuration configuration,DataBaseType dataBaseType) {
        ...
        try {
            ...
            if (isPKTypeValid(rsMetaData,dataBaseType)) {
                ...
                } else if (isLongType(rsMetaData.getColumnType(1), dataBaseType)) {
                    if(configuration != null) {
    ...
    private static boolean isPKTypeValid(ResultSetMetaData rsMetaData,DataBaseType dataBaseType) {
        boolean ret = false;
        try {
            int minType = rsMetaData.getColumnType(1);
            int maxType = rsMetaData.getColumnType(2);

            boolean isNumberType = isLongType(minType,dataBaseType);
    ...
    // Types.NUMERIC
    private static boolean isLongType(int type,DataBaseType dataBaseType) {
    ...
    public static List<String> genSplitSqlForOracle(String splitPK,
            ...
            int adviceNum,DataBaseType dataBaseType) {
        if (adviceNum < 1) {
        ...
        if (splitedRangeSize >= 2) {
            ...
            if (isLongType(splitedRange.get(0).getRight(), dataBaseType)) {
