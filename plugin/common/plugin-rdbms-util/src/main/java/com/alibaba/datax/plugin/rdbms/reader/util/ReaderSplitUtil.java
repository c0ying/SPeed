...
public final class ReaderSplitUtil {
    ...
    public static List<Configuration> doSplit(
        ...
        for (int i = 0, len = conns.size(); i < len; i++) {
			...
			DataBaseType dataBaseType = DataBaseType.dbNameOf(dbType);
            ...
            if (isTableMode) {
                ...
                if (needSplitTable) {
                    ...
                    for (String table : tables) {
                        ...
                        if (tempSlice.getString(Key.SPLIT_LOWER) != null && tempSlice.getString(Key.SPLIT_UPPER) != null) {
                        	boolean isStringType = Constant.PK_TYPE_STRING.equals(tempSlice.getString(Constant.PK_TYPE));
                            boolean isLongType = Constant.PK_TYPE_LONG.equals(tempSlice.getString(Constant.PK_TYPE));
                        	SplitInfoWrap splitInfoWrap = null;
                        	if (isStringType) {
								splitInfoWrap = new SplitInfoWrap(new ImmutablePair<Object, Object>(tempSlice.getString(Key.SPLIT_LOWER), tempSlice.getString(Key.SPLIT_UPPER)), 
										new ImmutablePair<Boolean, Boolean>(tempSlice.getBool(Key.SPLIT_LOWER_EQ), tempSlice.getBool(Key.SPLIT_UPPER_EQ)), "");
							}else if (isLongType) {
								splitInfoWrap = new SplitInfoWrap(new ImmutablePair<Object, Object>(new BigInteger(tempSlice.getString(Key.SPLIT_LOWER)), new BigInteger(tempSlice.getString(Key.SPLIT_UPPER))), 
										new ImmutablePair<Boolean, Boolean>(tempSlice.getBool(Key.SPLIT_LOWER_EQ), tempSlice.getBool(Key.SPLIT_UPPER_EQ)), "");
							}
                        	List<Configuration> splittedSlices = SingleTableSplitUtil.splitRangeTable(tempSlice, splitInfoWrap, eachTableShouldSplittedNumber, dataBaseType);
